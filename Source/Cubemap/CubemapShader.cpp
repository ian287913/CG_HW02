...
bool CubemapShader::Init(const std::vector<CubemapTexture>& textures, const std::vector<CubemapTexture>& textures2)
{
	...
	LoadTextures(textures2, textureID2);
	return LoadTextures(textures, textureID);
}
...
void CubemapShader::Render(int skyboxIndex)
{
...
void CubemapShader::UseTexture(bool use, int skyboxIndex)
{
	if (use)
	{
		if (skyboxIndex == 0)
...
bool CubemapShader::LoadTextures(const std::vector<CubemapTexture>& textures, GLuint& _textureID)
{
	if (textures.size() != 6)
	{
		return false;
