...
bool Sprite2D::Init(std::string fileName, int rowCount, int colCount, int FPS, bool scanRight, int rowGap, int colGap)
{
	...
	if (tdata.data != nullptr)
	{
		...
		if (scanRight)	//	scan right first
		{
			for (int row = rowCount - 1; row >= 0; --row)
			{
				for (int col = 0; col < colCount; ++col)
				{
					GLubyte *data = tdata.data + (row * tdata.width * (subHeight + rowGap) + col * (subWidth + colGap)) * 4;
